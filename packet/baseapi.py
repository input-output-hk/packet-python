...
class BaseAPI(object):
    ...
    def call_api(self, method, type="GET", params=None):  # noqa
        ...
        if not resp.ok and not (
            type == "POST" and api_check_skip and resp.status_code == 404
        ):
            msg = data
            if not data:
                msg = "(empty response)"
        ...
        elif not resp.ok and (
            type == "POST" and api_check_skip and resp.status_code == 404
        ):
            self._log.debug("Skipping ssh-key push error on packet.net")
        ...
        if not (type == "POST" and api_check_skip and resp.status_code == 404):
            ...
            self._log.debug(
                "Skipping raising an error for ssh-key push on packet.net"
            )
